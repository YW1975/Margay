...
const resolveWebUIPort = (config: WebUIUserConfig): number => {
  ...
  const envPort = parsePortValue(process.env.MARGAY_PORT ?? process.env.AIONUI_PORT ?? process.env.PORT, 'environment variable (MARGAY_PORT/AIONUI_PORT/PORT)');
...
const resolveRemoteAccess = (config: WebUIUserConfig): boolean => {
  const envRemote = parseBooleanEnv(process.env.MARGAY_ALLOW_REMOTE || process.env.AIONUI_ALLOW_REMOTE || process.env.AIONUI_REMOTE);
  const hostHint = (process.env.MARGAY_HOST ?? process.env.AIONUI_HOST)?.trim();
